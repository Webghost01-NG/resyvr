...
async function main() {
  ...
  requireEqual("Sepolia chain", (await sourceProvider.getNetwork()).chainId, BigInt(evidence.source.chainId));
  requireEqual("Creditcoin chain", (await destinationProvider.getNetwork()).chainId, BigInt(evidence.destination.chainId));

  const sourceReceipt = await sourceProvider.getTransactionReceipt(evidence.source.transactionHash);
  ...
  const destinationReceipt = await destinationProvider.getTransactionReceipt(evidence.destination.transactionHash);
  ...
  const sourceCode = await sourceProvider.getCode(evidence.source.address);
  ...
  const destinationCode = await destinationProvider.getCode(evidence.destination.address);
  ...
  requireEqual("V2 minimum activation stake", await destinationFactory.minimumIssuerBond(), evidence.destination.minimumBond);
  requireEqual(
    "V2 canonical source factory",
    await destinationFactory.SOURCE_VAULT_FACTORY(),
